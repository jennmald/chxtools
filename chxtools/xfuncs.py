...
version 0.2.0: fixed some problems with arrays as inputs, e.g. in get_gap
...
xdatafiles = [
    ...
    if (Path(datapath) / Path(f)).is_file()
]
...
def get_ac(material, E=8):
    ...
    calling sequence: ac=get_ac(material,E) where ac: critical angle in degrees, E [keV] (default: 8keV)
    ...
    if material in name:
        ...
        if np.min(E) >= np.min(n[:, 0] / 1000) and np.max(E) <= np.max(n[:, 0] / 1000):
            ...
            print(
                ...
                + "%3.4f" % E
                + "[keV] out of range ("
                + "%3.4f" % np.min(n[:, 0] / 1000)
                + "=<E<="
                + "%3.4f" % np.max(n[:, 0] / 1000)
                + "keV)"
            )
...
def get_T(material, E=8, thickness=1):
    ...
    if E.size == 1 or thickness.size == 1:
        if material in name:
            loadn = Path(datapath) / Path("n_{}.dat".format(material))
            n = pl.loadtxt(loadn, comments="%")
            if np.min(E) >= np.min(n[:, 0] / 1000) and np.max(E) <= np.max(
                n[:, 0] / 1000
            ):
                b = np.interp(E * 1000, n[:, 0], n[:, 2])
            ...
            print(
                'error: non recognized material, please create index of refraction file first. Type "get_T?" for instructions; type get_T("material?") for list of supported materials'
            )
    else:
        print(
            "error: either energy or length must be a scalar, cannot scan both energy and length at the same time."
        )


def get_Bragg(reflection, E=8.0):
    """
    by LW 17/03/2010
    ...
    if isinstance(
        ...
        if reflection in reflstr:
            ...
            print(
                'error: reflection not found. Type get_Bragg("reflections?") for list of available reflections.'
            )
    else:
        print(
...
def get_EBragg(reflection, theta_Bragg=12.0, d_spacing=None):
    ...
    if isinstance(
        ...
        if reflection in reflstr or d_spacing:
            ...
            print(
                'error: reflection not found. Type get_EBragg("reflections?") for list of available reflections.'
            )
...
def get_Qpixel(center=[0, 0], cord=[100, 100], dis=1000, ps=75, E=8):
    ...
    input parameters: position of the direct beam [x,y] in pixel numbers, pixel coordinate to calculate Q [x y],
...
def get_2theta(Qxy, E=8):
    """
    by LW 16/06/2005
    Returns the 2theta angle [deg] as a function of the input parameters Qxy [1/A] and Energy.
...
def get_Qll(Ty, alpha_i=0.12, d=1000, E=8):
...
def ch2deg(chn, channel_per_degree=100, chOff=0):
    ...
    """
    chn = np.array(chn)
    channel_per_degree = np.array(channel_per_degree)
...
def get_Vineyard(Qz, E=8, ra0=1):
    ...
    for i in range(0, np.size(Qz)):
        if rx[i] <= 1:
            ...
            V[i] = 2 * rx[i] / (rx[i] + np.sqrt(rx[i] ** 2 - 1))
    return V
...
def get_gap(E, harmonic=3, ID=default_id):
    ...
    if E == "ID?":
        print(
            "list of available magnetic measurements (based on data files in directory {}:".format(
                str(datapath)
            )
        )
        print(name)
        ...
        if ID in name:
            ...
            if np.min(E / harmonic) >= np.min(magdat[:, 2]) and np.max(
                E / harmonic
            ) <= np.max(magdat[:, 2]):
            ...
            # this else should be a warning only and should return NaN
                ...
                raise xfuncs_Exception(
                    "error: energy "
                    + "%3.4f" % E
                    + "[keV] out of range for requested harmonic number, gap limit: ("
                    + "%3.4f" % np.min(magdat[:, 0])
                    + "=<gap<="
                    + "%3.4f" % np.max(magdat[:, 0])
                    + "mm), try using higher/lower harmonic number."
                )
        else:
            raise xfuncs_Exception(
                "error: non recognized magnetic data. Type get_gap('ID?') for a list of available magnetic datasets."
            )
...
def get_Es(gap, harmonic=[1, 2, 3, 4, 5], ID=default_id):
    ...
    if gap == "ID?":
        print(
            "list of available magnetic measurements (based on data files in directory "
            + datapath
            + ":"
        )
        print(name)
        ...
        for idx in range(0, np.size(harmonic)):
            ...
            if idx == np.size(harmonic) - 1:
                ...
                if ID in name:
                    ...
                    if np.min(gap) >= np.min(magdat[:, 0]) and np.max(gap) <= np.max(
                        magdat[:, 0]
                    ):
                        ...
                        print(
                            "error: gap "
                            + "%3.4f" % gap
                            + "[mm] out of range for gap ("
                            + "%3.4f" % np.min(magdat[:, 0])
                            + "=<gap<="
                            + "%3.4f" % np.max(magdat[:, 0])
                            + "mm), try higher/lower harmonic number."
                        )
                else:
                    print(
                        "error: non recognized magnetic data. Type get_gap('ID?') for a list of available magnetic datasets."
...
def get_pinflux(current, Energy, thickness=300):
    ...
    if np.max(Energy) > xmax or np.min(Energy) < xmin:
        raise xfuncs_Exception(
            [
                "X-ray energy out of range for cross section data. ",
                xmin,
                "<=Energy<=,",
                xmax,
                " [keV]",
    ...
    PhCur = (
        ...
        * 1e3
        * 1.6022e-16
        * (1 - np.exp(-crossdat[:, 1] * thickness * rho_Si))
        / epsilon
    )
